...
Plugin URI: https://wordpress.dwbooster.com/calendars/cp-reservation-calendar
Description: This plugin allows you to easily insert reservation forms into your WP website.
Version: 1.1.41
Author: CodePeople
Author URI: https://bccf.dwbooster.com
License: GPL
Text Domain: cp-reservation-calendar
*/
...
function dex_reservations_activation_redirect( $plugin ) {
    if(
    ...
    {
        exit( wp_redirect( admin_url( 'admin.php?page=dex_reservations' ) ) );
    }
}
//add_action( 'activated_plugin', 'dex_reservations_activation_redirect' );
...
function _dex_reservations_install() {
    ...
    $sql = "CREATE TABLE `".$wpdb->prefix.DEX_RESERVATIONS_CONFIG_TABLE_NAME."` (".
                   ...
                   "PRIMARY KEY (`".TDE_RESERVATIONCONFIG_ID."`)) ".$charset_collate."; ";
    $wpdb->query($sql);
    ...
    $sql = "CREATE TABLE `".$wpdb->prefix.DEX_RESERVATIONS_CALENDARS_TABLE_NAME."` (".
                   ...
                   "PRIMARY KEY (`".TDE_RESERVATIONDATA_ID."`)) ".$charset_collate.";";
    $wpdb->query($sql);
...
function dex_reservations_helpLink($links) {
    $help_link = '<a href="https://wordpress.dwbooster.com/calendars/cp-reservation-calendar">'.__('Help').'</a>';
...
function dex_reservations_check_posted_data()
{
    ...
    if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_POST['dex_reservations_post_options'] ) && is_admin()  && current_user_can('edit_posts') )
    {       
    ...
    $buffer = sanitize_text_field($_POST["Date_s"] ." - ". $_POST["Date_e"]) ."\n".
    ...
    if (!wp_verify_nonce( $_REQUEST['nonce'], 'dexres_submit'))
    {
    ...
    if (!$rows_affected)
    {
        ...
        echo '<br /><br />If the error persists contact support service at https://wordpress.dwbooster.com/support';
        exit;
    }
...
function dex_reservations_check_IPN_verification() {
    ...
    dex_process_ready_to_go_reservation(intval($_GET["itemnumber"]), $payer_email);
...
function dex_process_ready_to_go_reservation($itemnumber, $payer_email = "")
{
   ...
   $myrows = $wpdb->get_results( "SELECT * FROM ".DEX_RESERVATIONS_TABLE_NAME." WHERE id=".intval($itemnumber) );

   $mycalendarrows = $wpdb->get_results( 'SELECT * FROM '.DEX_RESERVATIONS_CONFIG_TABLE_NAME .' WHERE `'.TDE_RESERVATIONCONFIG_ID.'`='.intval($myrows[0]->calendar));
...
function dex_reservations_save_options()
{
    ...
    if (!wp_verify_nonce( $_REQUEST['nonce'], 'dexres_update_data'))
    {
        echo 'Access error: nonce not verified. Contact our support service if you think this is an error';
        exit;
    }
...
function dex_reservations_calendar_load2() {
    ...
    $calid = intval(str_replace  (TDE_RESERVATIONCAL_PREFIX, "",$_GET["id"]));
    $query = "SELECT * FROM ".TDE_RESERVATIONCALENDAR_DATA_TABLE." where ".TDE_RESERVATIONDATA_IDCALENDAR."='".intval($calid)."'";
    ...
    {        
        ...
        echo $d1."-".$d2."\n";
        echo (is_admin() && current_user_can('edit_posts') ? $row[TDE_RESERVATIONDATA_TITLE] : 'Booked')."\n";
        echo (is_admin() && current_user_can('edit_posts') ? $row[TDE_RESERVATIONDATA_DESCRIPTION] : 'OK')."\n*-*\n";
    }
        ...
        {         
            ...
            foreach ($items as $item)
                ...
                {                            
                    ...
                    $wpdb->query("insert into ".TDE_RESERVATIONCALENDAR_DATA_TABLE."(".TDE_RESERVATIONDATA_IDCALENDAR.",".TDE_RESERVATIONDATA_DATETIME_S.",".TDE_RESERVATIONDATA_DATETIME_E.",".TDE_RESERVATIONDATA_TITLE.",".TDE_RESERVATIONDATA_DESCRIPTION.") values(".intval($calid).",'".esc_sql($datetime_s)."','".esc_sql($datetime_e)."','".esc_sql($title)."','".esc_sql($description)."') ");
                }    
        }
    }
