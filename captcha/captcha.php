...
function dexres_captcha_sanitize_key( $key ) {
   $key     = strtolower( $key );
...
$_SESSION['rand_code'.dexres_captcha_sanitize_key($_GET["ps"])] = str_replace(" ", "", $str);

setCookie('rand_code'.dexres_captcha_sanitize_key($_GET["ps"]), md5(str_replace(" ", "", $str)), time()+36000,"/");
