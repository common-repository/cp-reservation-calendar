<?php
add_action( 'admin_enqueue_scripts', 'cprescal_feedback_insert_adminScripts', 1); 
add_action( 'wp_ajax_cprescal_feedback', 'cprescal_feedback' );
$cprescal_plugfile = 'dex_reservations.php';
$cprescal_plugslug = 'cp-reservation-calendar';
$cprescal_supportlink = 'https://wordpress.dwbooster.com/support';
$cprescal_supportlink_full = $cprescal_supportlink . '?priority-support=yes';

if (!get_option('installed_'.$cprescal_plugslug, ''))
    update_option('installed_'.$cprescal_plugslug, time() );

function cprescal_feedback_insert_adminScripts($hook) {
    if( 'plugins.php' == $hook  )            
    {
        wp_enqueue_style('wp-jquery-ui-dialog');
        wp_enqueue_script('jquery');
        wp_enqueue_script('jquery-ui-dialog');       
        add_action( 'admin_footer', 'cprescal_feedback_javascript' );            
    }
}
...
function cprescal_feedback() {
    global $cprescal_plugfile, $cprescal_plugslug;
    $cprescal_plugin_data = get_plugin_data( dirname(__FILE__).'/'.$cprescal_plugfile );
    $cprescal_plugin_version = $cprescal_plugin_data['Version'];
    $time = time() - get_option('installed_'.$cprescal_plugslug, '');
    $data = array(
                 'answer' => (@$_POST["answer"]),
                 'otherplugin' => (@$_POST["opinfo"]),
                 'otherinfo' => (@$_POST["oinfo"]),
                 'plugin' => ($cprescal_plugin_data['Name']),
                 'pluginv' => ($cprescal_plugin_version),
...
function cprescal_feedback_javascript() { 
    global $cprescal_plugslug, $cprescal_supportlink, $cprescal_supportlink_full; 
      ?>
	<script type="text/javascript">

$ = jQuery.noConflict()
$( window ).load(function() {
        document.querySelector('[data-slug="<?php echo $cprescal_plugslug; ?>"] .deactivate a').addEventListener('click', function(event){
            event.preventDefault()
            var urlRedirect = document.querySelector('[data-slug="<?php echo $cprescal_plugslug; ?>"] .deactivate a').getAttribute('href');
            ...
            $('<div title="QUICK FEEDBACK"><div style="padding:10px;">'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cprescal_update_reason(this);" value="no-work"> The plugin didn\'t work<br />'+
                '<div id="abcm_nowork" style="margin-left:25px;display:none;padding:10px;border:1px dotted gray;color:#660000"><strong>We can help!</strong> We offer <strong>free support</strong> for this plugin. Feel free to open a support ticket at <a href="<?php echo $cprescal_supportlink_full; ?>"><strong><?php echo $cprescal_supportlink; ?></strong></a> and we will be happy to help.</div></div>'+
                
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cprescal_update_reason(this);" value="-"> I don\'t like to share my information with you<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cprescal_update_reason(this);" value="temporary-deactivation"> It\'s a temporary deactivation. I\'m just debugging an issue.<br /></div>'+
               
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cprescal_update_reason(this);" value="better-plugin"> I found a better plugin<br />'+
               ...
               '<div class="abcreasonblock"><input type="radio" name="abcm_reason" onclick="cprescal_update_reason(this);" value="other"> Other<br />'+
      		   ...
      		   '</div></div>'
                  ...
                  buttons: [
                      ...
                      },
                      {
                        ...
                        click: function() {
                                   window.open('<?php echo $cprescal_supportlink_full; ?>');
      						       $(this).dialog("close");
      				           }                            
                      },
                      {
...
function cprescal_update_reason(field)
{
